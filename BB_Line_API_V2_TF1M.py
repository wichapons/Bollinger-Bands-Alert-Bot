...
while True:
    try:
        ...
        if upper_bb[-1] - lower_bb[-1] < middle_bb[-1]:
            ...
            if low_prices_tf_1m[-1] > upper_bb[-1]:
                # send the alert via Line API
                send_line_alert(line_token, f"{symbol}, price: {low_prices_tf_1m[-1]}, candle tf 1m hit upper bound Bollinger Band line on tf 15 m")

            # check if the current candle hits the lower bound Bollinger Band line
            if low_prices_tf_1m[-1] < lower_bb[-1]:
                # send the alert via Line API
                send_line_alert(line_token, f"{symbol}, price: {low_prices_tf_1m[-1]}, candle tf 1m hit lower bound Bollinger Band line on tf 15 m")
