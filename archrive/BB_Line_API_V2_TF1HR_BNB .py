...
while True:
    try:
        ...
        candle_data_tf_15m = client.futures_klines(symbol=symbol, interval="15m", limit=bb_period+1)
        ...
        low_prices_tf_15m = np.array([float(x[4]) for x in candle_data_tf_15m])
        ...
        if upper_bb[-1] - lower_bb[-1] < middle_bb[-1]:
            ...
            if low_prices_tf_15m[-1] > middle_bb[-1]:
            ...
            if low_prices_tf_15m[-1] < lower_bb[-1]:
                # send the alert via Line API
